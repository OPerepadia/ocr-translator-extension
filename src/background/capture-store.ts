...
/** Same contract without the persistence, for tests and for contexts that
 * refuse IndexedDB. Captures are then lost on unload, as they were before. */
export function createMemoryCaptureStore(
  options: Pick<CaptureStoreOptions, "now"> = {},
): CaptureStore {
  const now = options.now ?? Date.now;
  const records = new Map<string, CaptureRecord>();

  function unexpired(frameKey: string): CaptureRecord | undefined {
    const record = records.get(frameKey);
    if (record && record.updatedAt <= now() - CAPTURE_TTL_MS) {
      records.delete(frameKey);
      return undefined;
    }
    return record;
  }
  ...
  return {
    async get(frameKey) {
      return unexpired(frameKey);
    },

    async update(frameKey, mutate) {
      const next = mutate(unexpired(frameKey));
      if (!next) {
        return;
      }

      const expiresBefore = now() - CAPTURE_TTL_MS;
      for (const [key, record] of records) {
        if (record.updatedAt <= expiresBefore) {
          records.delete(key);
        }
      }

      // Re-inserting keeps iteration order equal to write order, which the
      // eviction below reads as oldest-first.
      records.delete(frameKey);
      records.set(frameKey, { ...next, updatedAt: now() });
      for (const [key] of [...records].slice(0, excess(records.size))) {
        records.delete(key);
      }
    },
  };
